...
set_time_limit(240);
define('POPUP_TITLE','Media Browser');
$tinybrowser = array();
...
if(isset($_COOKIE['instance'])) {
    ...
    $destFolder = isset($_SESSION['tiny_upload'][$instance]) ? $_SESSION['tiny_upload'][$instance] : null;
}
...
$tinybrowser['path']['default_root'] = '/shameer/deal/uploads/';
...
if(isset($destFolder)){
    ...
    $uploadRoot = $tinybrowser['path']['default_root'];
}
// File upload paths (set to absolute by default)
$tinybrowser['path']['image'] = $uploadRoot.'images/'; // Image files location - also creates a '_thumbs' subdirectory within this path to hold the image thumbnails
$tinybrowser['path']['media'] = $uploadRoot.'medias/';  // Media files location
$tinybrowser['path']['file']  = $uploadRoot.'files/';  // Other files location
